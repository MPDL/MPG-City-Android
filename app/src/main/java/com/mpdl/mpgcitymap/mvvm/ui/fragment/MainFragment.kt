...
class MainFragment : BaseFragment<MainViewModel>(), Scene.OnUpdateListener, View.OnClickListener {
    ...
    private lateinit var webPopupRootView:LinearLayout
    ...
    override fun initData(savedInstanceState: Bundle?) {
        ...
        width = wm.defaultDisplay.width - 100
        height = wm.defaultDisplay.height - 300
    }
...
//                    showNode(augmentedImage)
    ...
    private fun createWebDialog(url: String){
        if (webDialog != null){
            webDialog?.dismiss()
            webPopupRootView?.findViewById<LinearLayout>(R.id.web_parent).removeAllViewsInLayout()
            webDialog = null
        }

        val webView = createWebView(url)
        webPopupRootView = View.inflate(requireActivity(),R.layout.popup_web,null) as LinearLayout
        webPopupRootView.findViewById<LinearLayout>(R.id.web_parent).addView(webView)
        webPopupRootView.findViewById<TextView>(R.id.tv_close).setOnClickListener { _ ->
            webDialog?.dismiss()
            webPopupRootView.findViewById<LinearLayout>(R.id.web_parent).removeAllViewsInLayout()
        }

        webDialog = AlertDialog.Builder(requireContext())
                .setView(webPopupRootView)
                .create()
        webView.loadUrl(url)
        showWebDialog(true)
    }

    private fun showWebDialog(create: Boolean){
        if (webDialog == null){
            return
        }
        if (create || webDialog?.isShowing!!){
            webDialog?.show()
            val lp = webDialog?.window?.attributes
            ...
            webDialog?.window?.attributes = lp
        }
    }
    ...
    private fun initListCity(){
        ...
        cityItemList.add(CustomItem(R.id.item0,R.mipmap.city_map_1,-1,0,"https://www.youtube.com/watch?v=zZFyD8kSaIo"))
        ...
        cityItemList.add(CustomItem(R.id.item4,R.mipmap.city_map_4,1,-1,"https://www.mpg.de/197521/statutesMPS.pdf"))
        cityItemList.add(CustomItem(R.id.item5,R.mipmap.city_map_5,-1,-1,"https://www.archiv-berlin.mpg.de/2523/en"))
        cityItemList.add(CustomItem(R.id.item6,R.mipmap.city_map_6,-2,0,0,"item_6_content.jpg"))
        ...
        cityItemList.add(CustomItem(R.id.item9,R.mipmap.city_map_9,0,2,"https://www.youtube.com/watch?v=X2yDs5FHDok&feature=youtu.be"))
        cityItemList.add(CustomItem(R.id.item10,R.mipmap.city_map_10,2,2,"https://keeper.mpdl.mpg.de/f/fd6f3edc5ea64ce09020/"))
        ...
        cityItemList.add(CustomItem(R.id.item14,R.mipmap.city_map_14,2,-2,"https://www.gnz.mpg.de/index.php/Hauptseite/en"))
    ...
    private fun createWebView(url:String):WebView{
        ...
        webView.webChromeClient = object :WebChromeClient(){
            override fun onReceivedTitle(view: WebView?, title: String?) {
                ...
                title?.let {
                    webPopupRootView.findViewById<TextView>(R.id.tv_title).text = it
                }
            }

            override fun onProgressChanged(view: WebView?, newProgress: Int) {
                super.onProgressChanged(view, newProgress)
                webPopupRootView.findViewById<ProgressBar>(R.id.progressbar_web).progress = newProgress
            }
        }
